...
const (
	// CoinType defines CRESCENT bip44 coin type
	CoinType = uint32(999)
	// FullFundraiserPath defines full fundraiser path for CRESCENT coin type
	FullFundraiserPath = "44'/999'/0'/0/0"
...
func (a appCreator) appExport(
	...
	if height != -1 {
		farmingApp = app.NewFarmingApp(logger, db, traceStore, false, map[int64]bool{}, homePath, uint(1), a.encCfg, appOpts)

		if err := farmingApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		farmingApp = app.NewFarmingApp(logger, db, traceStore, true, map[int64]bool{}, homePath, uint(1), a.encCfg, appOpts)
	}
