...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msgDeposit := liquiditytypes.NewMsgDeposit(
				recipient,
				1,
				sdk.NewCoins(
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			recipient := clientCtx.GetFromAddress()
			direction := liquiditytypes.OrderDirectionSell
			...
			msgSwap := &liquiditytypes.MsgLimitOrder{
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msgLiquidStake := liquidstakingtypes.NewMsgLiquidStake(
				recipient,
