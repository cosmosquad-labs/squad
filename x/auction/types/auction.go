...
func NewBaseAuction(
	...
	startPrice sdk.Dec, sellingCoins sdk.Coins, payingCoinDenom string,
...
func (ba BaseAuction) GetSellingCoins() sdk.Coins {
	return ba.SellingCoins
}

func (ba *BaseAuction) SetSellingCoins(coins sdk.Coins) error {
	ba.SellingCoins = coins
	return nil
}
...
func (ba BaseAuction) Validate() error {
	if ba.Type != AuctionTypeFixedPrice {
	...
	if err := ba.SellingCoins.Validate(); err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidCoins, "invalid selling coins: %v", ba.SellingCoins)
	}
	...
	if err := ValidateVestingSchedules(ba.VestingSchedules, ba.EndTime); err != nil {
		return err
	}
	return nil
}
...
func (ba BaseAuction) ShouldAuctionClosed(t time.Time) bool {
	return !ba.GetEndTime().After(t) // LastEndTime <= Time
}

// NewFixedPriceAuction returns a new fixed price auction.
func NewFixedPriceAuction(baseAuction *BaseAuction, remainingSellingCoins sdk.Coins) *FixedPriceAuction {
	return &FixedPriceAuction{
		BaseAuction:           baseAuction,
		RemainingSellingCoins: remainingSellingCoins,
	}
}
...
type AuctionI interface {
	...
	SetSellingCoins(sdk.Coins) error
