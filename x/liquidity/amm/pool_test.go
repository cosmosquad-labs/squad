...
func TestBasicPool(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		buyAmt := pool.BuyAmountOver(defTickPrec.LowestTick())
		expected := rx.ToDec().QuoRoundUp(defTickPrec.LowestTick())
		require.True(t, utils.DecApproxEqual(expected, buyAmt.ToDec()))
		sellAmt := pool.SellAmountUnder(defTickPrec.HighestTick())
...
func TestBasicPool_Price(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			pool := amm.NewBasicPool(sdk.NewInt(tc.rx), sdk.NewInt(tc.ry), sdk.Int{})
...
func TestRangedPool(t *testing.T) {
	r := rand.New(rand.NewSource(0))
	for i := 0; i < 1000; i++ {
		x, y := sdk.NewInt(1+r.Int63n(100000000)), sdk.NewInt(1+r.Int63n(100000000))
		initialPrice := utils.RandomDec(r, utils.ParseDec("0.05"), utils.ParseDec("500.0"))
		minPrice := utils.ParseDecP("0.1")
		maxPrice := utils.ParseDecP("100.0")
		pool, err := amm.CreateRangedPool(x, y, initialPrice, minPrice, maxPrice)
		...
		buyAmt := pool.BuyAmountOver(defTickPrec.LowestTick())
		expected := rx.ToDec().QuoTruncate(*minPrice).TruncateInt()
		require.True(t, expected.Sub(buyAmt).LTE(sdk.OneInt()))
		sellAmt := pool.SellAmountUnder(defTickPrec.HighestTick())
		require.True(t, ry.Sub(sellAmt).LTE(sdk.OneInt()))
	}
}

func TestRangedPool_Price(t *testing.T) {
	for _, tc := range []struct {
		name               string
		rx, ry             int64 // reserve balance
	...
	}{
		{
			...
			rx:       20000,
			ry:       100,
			minPrice: utils.ParseDecP("0.01"), // TODO: TBD
			maxPrice: utils.ParseDecP("100.0"),
			p:        sdk.NewDec(200),
		},
		{
			...
			rx:       200,
			ry:       300,
			minPrice: utils.ParseDecP("0.01"), // TODO: should be updated
			maxPrice: utils.ParseDecP("100.0"),
			p:        sdk.MustNewDecFromStr("0.666666666666666667"),
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			pool := amm.NewRangedPool(sdk.NewInt(tc.rx), sdk.NewInt(tc.ry), sdk.Int{}, tc.minPrice, tc.maxPrice)
			require.True(sdk.DecEq(t, tc.p, pool.Price()))
		})
	}
...
func TestRangedPool_IsDepleted(t *testing.T) {
	for _, tc := range []struct {
		name               string
		rx, ry             int64 // reserve balance
		minPrice, maxPrice *sdk.Dec
		...
		t.Run(tc.name, func(t *testing.T) {
			pool := amm.NewRangedPool(sdk.NewInt(tc.rx), sdk.NewInt(tc.ry), sdk.NewInt(tc.ps), tc.minPrice, tc.maxPrice)
			require.Equal(t, tc.isDepleted, pool.IsDepleted())
		})
	}
}
...
func TestRangedPool_BuyAmountOverOverflow(t *testing.T) {
	n, _ := sdk.NewIntFromString("10000000000000000000000000000000000000000000")
	pool := amm.NewRangedPool(n, sdk.NewInt(1000), sdk.Int{}, nil, utils.ParseDecP("1000000")) // TODO: is maxPrice correct?
	amt := pool.BuyAmountOver(defTickPrec.LowestTick())
	require.True(sdk.IntEq(t, amm.MaxCoinAmount, amt))
}
