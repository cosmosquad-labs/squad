...
func BenchmarkOrderBook_Add(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		ob.Add(types.NewOrder(orderer, types.SwapDirectionXToY, sdk.OneDec(), sdk.OneInt()))
	}
}
...
func TestOrderBook_Add(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		for j := 0; j < 10; j++ {
			ob = ob.Add(types.NewOrder(orderer, types.SwapDirectionXToY, price, sdk.NewInt(100)))
