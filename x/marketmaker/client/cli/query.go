...
func GetCmdQueryParams() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "params",
		Args:  cobra.NoArgs,
		Short: "Query the current marketmaker parameters information",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Query values set as marketmaker parameters.
Example:
$ %s query %s params
`,
...
// NewQueryMarketMakerCmd implements the market maker query command.
func NewQueryMarketMakerCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "marketmaker",
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queryClient := types.NewQueryClient(clientCtx)
			var req *types.QueryMarketMakersRequest
