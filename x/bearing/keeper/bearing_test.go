...
func (suite *KeeperTestSuite) TestCollectBearings() {
	for _, tc := range []struct {
		...
		{
			"disabled bearing epoch with bearings",
			suite.bearings[:4],
			0,
			[]sdk.AccAddress{
				suite.destinationAddrs[0],
...
func (suite *KeeperTestSuite) TestBearingChangeSituation() {
	...
	for _, tc := range []struct {
		...
		{
			"add bearing 3 with invalid total rate case 1",
			testProposal(proposal.ParamChange{
				Subspace: types.ModuleName,
				Key:      string(types.KeyBearings),
				Value: `[
		...
		{
			"add bearing 3 with invalid total rate case 2",
			testProposal(proposal.ParamChange{
				Subspace: types.ModuleName,
				Key:      string(types.KeyBearings),
				Value: `[
		...
		{
			"add bearing 4 without date range overlap",
			testProposal(proposal.ParamChange{
				Subspace: types.ModuleName,
				Key:      string(types.KeyBearings),
				Value: `[
		...
		suite.Run(tc.name, func() {
			...
			// endblock gov paramchange ->(new block)-> beginblock bearing -> mempool -> endblock gov paramchange ->(new block)-> ...
