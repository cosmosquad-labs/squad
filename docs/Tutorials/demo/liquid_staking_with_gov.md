...
export VALIDATOR_2="midnight shove devote endless olive describe leg width awkward next grain earn demise ketchup write notice adult setup cloud exile fine garbage tuna dice"
export VALIDATOR_3="outdoor victory trap million pet unfair mail together lift one balcony gospel endorse glide shield throw soul moral only elite genius ethics title chase"
export VALIDATOR_4="pony produce own offer essence hover task crime shell romance pave unfold hope gentle wrist vague hello voice drama tomato drama fabric glance useless"
export VALIDATOR_5="boss antenna first twice culture unhappy atom tooth scorpion satoshi chicken twelve army fresh noodle north clog range require romance sentence switch inhale isolate"
export USER_A="guard cream sadness conduct invite crumble clock pudding hole grit liar hotel maid produce squeeze return argue turtle know drive eight casino maze host"
export USER_B="fuel obscure melt april direct second usual hair leave hobby beef bacon solid drum used law mercy worry fat super must ritual bring faculty"
export USER_C="identify violin unique scrub elevator cash retire chicken invest special ill whisper police still anchor put scare fix absurd upgrade myself render confirm special"
export USER_D="style artefact seed weather cost museum census gown announce captain output change mention march jealous pond salad want fish radio anger auction design hockey"
export USER_E="amount iron dial paddle quiz possible enhance noble cup sustain plunge bitter auction umbrella myself bacon current battle caught swallow match slice detail cute"
export USER_F="ribbon unaware nothing jar virtual build state action bamboo busy rose keen pulse ripple feature rally body slender victory purity cradle vacant history clean"
export USER_G="crowd farm cake dry weasel panther float reflect annual give display power home marble auto frown host alcohol jewel vivid swarm tobacco chair spike"
export VALIDATOR_GENESIS_COINS=100000000000000000stake,10000000000uatom,10000000000uusd
export USER_GENESIS_COINS=10000000000stake,10000000000uatom,10000000000uusd,10000000000poolD35A0CC16EE598F90B044CE296A405BA9C381E38837599D96F2F70C2F02A23A4,10000000000pool93E069B333B5ECEBFE24C6E1437E814003248E0DD7FF8B9F82119F4587449BA5,10000000000pool3036F43CB8131A1A63D2B3D3B11E9CF6FA2A2B6FEC17D5AD283C25C939614A8C,10000000000poolE4D2617BFE03E1146F6BBA1D9893F2B3D77BA29E7ED532BB721A39FF1ECC1B07
...
echo $VALIDATOR_3 | $BINARY keys add val3 --keyring-backend test --recover
echo $VALIDATOR_4 | $BINARY keys add val4 --keyring-backend test --recover
echo $VALIDATOR_5 | $BINARY keys add val5 --keyring-backend test --recover
echo $USER_A | $BINARY keys add userA --keyring-backend test --recover
echo $USER_B | $BINARY keys add userB --keyring-backend test --recover
echo $USER_C | $BINARY keys add userC --keyring-backend test --recover
echo $USER_D | $BINARY keys add userD --keyring-backend test --recover
echo $USER_E | $BINARY keys add userE --keyring-backend test --recover
echo $USER_F | $BINARY keys add userF --keyring-backend test --recover
echo $USER_G | $BINARY keys add userG --keyring-backend test --recover
$BINARY add-genesis-account $($BINARY keys show val1 --keyring-backend test -a) $VALIDATOR_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show val2 --keyring-backend test -a) $VALIDATOR_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show val3 --keyring-backend test -a) $VALIDATOR_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show val4 --keyring-backend test -a) $VALIDATOR_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show val5 --keyring-backend test -a) $VALIDATOR_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show userA --keyring-backend test -a) $USER_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show userB --keyring-backend test -a) $USER_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show userC --keyring-backend test -a) $USER_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show userD --keyring-backend test -a) $USER_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show userE --keyring-backend test -a) $USER_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show userF --keyring-backend test -a) $USER_GENESIS_COINS
$BINARY add-genesis-account $($BINARY keys show userG --keyring-backend test -a) $USER_GENESIS_COINS
$BINARY gentx val1 10000000stake --moniker val1 --chain-id $CHAIN_ID --keyring-backend test
#$BINARY gentx val2 10000000stake --moniker val2 --node-id val2 --chain-id $CHAIN_ID --keyring-backend test
#$BINARY gentx val3 10000000stake --moniker val3 --node-id val3 --chain-id $CHAIN_ID --keyring-backend test
#$BINARY gentx val4 10000000stake --moniker val4 --node-id val4 --chain-id $CHAIN_ID --keyring-backend test
#$BINARY gentx val5 10000000stake --moniker val5 --node-id val5 --chain-id $CHAIN_ID --keyring-backend test
